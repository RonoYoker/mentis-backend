...
def offer_segment_callback_processor(parent_id: str, url: str):
    """
    communicates with onyx(source/client) with the callback data for the
    two queries executed to update data for a segment
    """
    # from onyx_proj.apps.async_task_invocation.async_tasks_callback_processor import offer_segment_callback_processor
    logger.debug(f"offer_segment_callback_processor :: parent_id: {parent_id}, url: {url}")
    ...
    if request_data["error"] is True or len(request_data.get("result", [])) == 0:
        logger.error(f"offer_segment_callback_processor :: Unable to fetch Project_id data for parent_id: {parent_id}.")
    else:
        request_data = request_data["result"]

    if len(tasks_data.get("result", [])) == 0 or tasks_data is None:
        logger.error(f"offer_segment_callback_processor :: No tasks split for the parent_id: {parent_id}.")
    ...
    api_response = json.loads(
        RequestClient(request_type="POST", url=settings.ETHERA_COMMONS_DOMAIN + url, request_body=json.dumps(request_payload)).get_api_response())

    logger.debug(f"offer_segment_callback_processor :: callback api_response is: {api_response}")
    # from onyx_proj.apps.segments.segments_processor.segment_callback_processor import process_segment_callback
    # response = process_segment_callback(request_payload)

    if api_response.get("result") == TAG_FAILURE:
        logger.error(
            f"offer_segment_callback_processor :: Callback_failed for parent_id: {parent_id}. API Response: {api_response}")
        status = AsyncJobStatus.CALLBACK_FAILED.value
        error_message = api_response
    else:
        logger.debug(
            f"offer_segment_callback_processor :: Callback_success for parent_id: {parent_id}. API Response: {api_response}")
        status = AsyncJobStatus.SUCCESS.value
        error_message = None
    ...
    db_resp = CEDQueryExecution().update_status_dict({"Status": status, "ErrorMessage": error_message}, parent_id)
    if db_resp.get("success", False) is True:
        logger.info(f"offer_segment_callback_processor :: Updated status to {status} for parent_id: {parent_id}.")
    else:
        logger.error(
            f"offer_segment_callback_processor :: Unable to update status to {status} for parent_id: {parent_id}.")

    return dict(status=status)


def offer_data_population_callback_processor(parent_id: str, url: str):
    """
    s3_output mode callback to populate offers data. Client is EtheraCommons.
    """
    # from onyx_proj.apps.async_task_invocation.async_tasks_callback_processor import offer_segment_callback_processor
    logger.debug(f"offer_data_population_callback_processor :: parent_id: {parent_id}, url: {url}")
    ...
    if request_data["error"] is True or len(request_data.get("result", [])) == 0:
        logger.error(f"offer_data_population_callback_processor :: Unable to fetch Project_id data for parent_id: {parent_id}.")
    else:
        request_data = request_data["result"]

    if len(tasks_data.get("result", [])) == 0 or tasks_data is None:
        logger.error(f"offer_data_population_callback_processor :: No tasks split for the parent_id: {parent_id}.")
    ...
    api_response = json.loads(
        RequestClient(request_type="POST", url=settings.ETHERA_COMMONS_DOMAIN + url,
                      request_body=json.dumps(request_payload)).get_api_response())

    logger.debug(f"offer_data_population_callback_processor :: callback api_response is: {api_response}")
    # from onyx_proj.apps.segments.segments_processor.segment_callback_processor import process_segment_callback
    # response = process_segment_callback(request_payload)

    if api_response.get("result") == TAG_FAILURE:
        logger.error(
            f"offer_data_population_callback_processor :: Callback_failed for parent_id: {parent_id}. API Response: {api_response}")
        status = AsyncJobStatus.CALLBACK_FAILED.value
        error_message = api_response
    else:
        logger.debug(
            f"offer_data_population_callback_processor :: Callback_success for parent_id: {parent_id}. API Response: {api_response}")
        status = AsyncJobStatus.SUCCESS.value
        error_message = None
    ...
    db_resp = CEDQueryExecution().update_status_dict({"Status": status, "ErrorMessage": error_message}, parent_id)
    if db_resp.get("success", False) is True:
        logger.info(f"offer_data_population_callback_processor :: Updated status to {status} for parent_id: {parent_id}.")
    else:
        logger.error(
            f"offer_data_population_callback_processor :: Unable to update status to {status} for parent_id: {parent_id}.")

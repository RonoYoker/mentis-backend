...
from onyx_proj.apps.slot_management.app_settings import CAMPAIGN_THRESHOLDS_PER_MINUTE, SLOT_INTERVAL_MINUTES
from onyx_proj.common.constants import TAG_SUCCESS
...
def vaildate_campaign_for_scheduling(request_data):
    ...
    for campaign in campaigns_list:
        ...
        campaigns_date_type_data.setdefault(key, [])
    ...
    curr_campaigns = CED_CampaignBuilderCampaign().get_campaigns_segment_info_by_dates([seg_date.strftime("%Y-%m-%d") for seg_date in dates_to_validate])
...
def validate_schedule(schedule,content_type):
    slot_limit = CAMPAIGN_THRESHOLDS_PER_MINUTE[content_type] * SLOT_INTERVAL_MINUTES
    curr_segments_map = {}

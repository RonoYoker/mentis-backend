...
def get_user_projects_data():
    method_name = "get_user_projects_data"
    logger.debug(f"LOG_ENTRY function name : {method_name}")
...
def update_user_projects_data(request_data):
    method_name = "update_user_projects_data"
    logger.debug(f"LOG_ENTRY function name : {method_name}")
...
def send_user_password_email(user_name , password, first_name, last_name, email_id):
    ...
    status = email_utility().send_mail(tos, ccs, bccs, email_subject, email_body)
    if status.get("status_code") != http.HTTPStatus.OK:

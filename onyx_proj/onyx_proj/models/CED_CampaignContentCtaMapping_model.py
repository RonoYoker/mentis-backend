...
class CEDCampaignContentCtaMapping:
    def __init__(self, **kwargs):
        self.database = kwargs.get("db_conf_key", "default")
        self.table_name = "CED_CampaignContentCtaMapping"
        self.curr = mysql_connect(self.database)
        self.table = CED_CampaignContentCtaMapping
        self.engine = sql_alchemy_connect(self.database)

    def save_or_update_content_cta_mapping_details(self, cta_mapping):
        try:
            res = save_or_update_merge(self.engine, cta_mapping)
        except Exception as ex:
            return dict(status=False, response=str(ex))
        return dict(status=True, response=res)

    def delete_content_cta_mapping(self, content_id):
        filter_list = [
    ...
    def get_content_and_cta_mapping_data(self, content_id, cta_id, content_type):
        filter_list = [
            {"column": "content_id", "value": content_id, "op": "=="},
            {"column": "is_active", "value": 1, "op": "=="},
            {"column": "is_deleted", "value": 0, "op": "=="},
            {"column": "url_id", "value": cta_id, "op": "=="},

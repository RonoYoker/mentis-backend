...
class CEDCampaignEmailContent:
    ...
    def get_email_content_data_by_unique_id_and_status(self, unique_id, status_list):
        ...
        res = fetch_rows_limited(self.engine, self.table, filter_list, ["id", "unique_id", "title", "strength",
                                            "project_id", "is_deleted", "history_id", "extra", "vendor_template_id",
                                            "created_by", "approved_by", "status", "is_contain_url","language_name",
                                            "is_active", "rejection_reason", "creation_date", "updation_date", "template_category"],
    ...
    def filter_cb_with_no_template_category(self, campaigns_id_list):
        query = f"""
                ...
                JOIN CED_CampaignBuilderEmail cbs ON cbc.UniqueId = cbs.MappingId
                JOIN CED_CampaignEmailContent csc ON cbs.EmailId = csc.UniqueId
